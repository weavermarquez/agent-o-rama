...
public interface AgentGraph {
  ...
  <%= (mk-full-type-decl i) %> AgentGraph node(String name, Object outputNodesSpec, RamaVoidFunction<%= (inc i) %><%= (mk-full-type-decl ["AgentNode"] i []) %> impl);<% )) %><% (dofor [i (range 0 (dec MAX-ARITY))] (str %>
